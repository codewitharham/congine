...
- **Old (pre-session):** *Not safe to ship to enterprise or open-source today.*
...
- The cleartext warning in `dependency_injection.py:99-104` is preserved as an *audit trail of a conscious opt-out*: it fires on any non-local cleartext URL — which the deployer can only reach by explicitly setting `allow_cleartext=True` (otherwise `validate()` would have already raised).
