...
def setup_secure_container() -> ServiceContainer:
    ...
    if not os.path.isabs(contracts_folder):
        os.environ["CONGINE_LOCAL_CONTRACTS_DIR"] = str(
            (base_dir / contracts_folder).resolve()
        )
...
def process_untrusted_customer_ticket(
    ticket_id: str, raw_customer_email: str
) -> Dict[str, Any]:
    """
...
def run_real_world_scenarios() -> None:
    ...
    try:
        leaked_output = process_untrusted_customer_ticket(
            "TKT-ATTACK", malicious_injection
        )
        ...
        print(
            "   🛡️ Security Shield Activated! CongineSDK caught a runtime data breach."
        )
        ...
        if llm:
            try:
                raw_hacked_payload = llm.with_structured_output(
                    ReturnTicketSchema
                ).invoke(malicious_injection)
                dissect_and_print_breaches(
                    RETURN_CONTRACT, raw_hacked_payload.model_dump()
                )
            except Exception:
