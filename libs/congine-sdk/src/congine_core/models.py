"""Canonical value contracts (Layer 0 kernel).

Pure, frozen data structures with zero framework dependencies and — critically —
zero *internal* dependencies: this module imports only the standard library.

**P1 architectural clarification.** These types previously lived at
``congine_core.domain.models`` (L2). They are not judgment logic; they are the
canonical value vocabulary that every layer exchanges, so L1 ports, L4
infrastructure and L5 adapters all legitimately need them. Classifying them as
L2 forced ports and infrastructure into forbidden outward dependencies that the
architecture gate had to ignore. Placing them in L0 — beside :mod:`config`,
:mod:`exceptions` and :mod:`security_limits` — makes those edges inward and
lawful, so the gate can reject ``L1 -> L2`` and ``L4 -> L2`` with no exemptions.

The split is now: **L0 owns canonical cross-layer value contracts; L2 owns
deterministic judgment and rules** (``RuleEngine``, the validators, contract
admission, schema vocabulary).

``congine_core.domain.models`` remains as a compatibility re-export. Note that
importing through that path keeps its L2 identity for architecture purposes, so
an L1 or L4 module must import ``congine_core.models`` directly.

All four dataclasses here are immutable (``frozen=True``); :class:`TelemetryEvent`
uses ``__post_init__`` defaulting via ``object.__setattr__``.
"""
